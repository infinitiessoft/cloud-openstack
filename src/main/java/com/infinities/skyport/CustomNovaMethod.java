...
public class CustomNovaMethod extends NovaMethod {
	...
	public synchronized @Nullable AuthenticationContext authenticate() throws CloudException, InternalException {
		...
		try {
			...
			if (endpoint.startsWith("ks:")) {
				...
				if (endpoint.endsWith("1.0") || endpoint.endsWith("1.0/") || endpoint.endsWith("1.1")
						|| endpoint.endsWith("1.1/")) {
					auth = authenticateStandard(endpoint);
					if (auth == null) {
	...
	private @Nullable AuthenticationContext authenticateKeystone(@Nonnull String endpoint) throws CloudException,
			InternalException {
		Logger std = NovaOpenStack.getLogger(NovaOpenStack.class, "std");
			...
			if (code != HttpStatus.SC_OK) {
				...
				if (data != null && !data.trim().equals("")) {
					...
					if (id != null) {
						...
						for (int i = 0; i < catalog.length(); i++) {
							try {
								...
								for (int j = 0; j < endpoints.length(); j++) {
									...
									String url = test.getString(urlType);
									...
									if (url != null) {
										...
										if (version == null || version.equals("")) {
											std.debug("No versionId parameter... Parsing URL " + url
													+ " for best guess.  (vSadTrombone)");
										...
										if (NovaOpenStack.isSupported(version)) {
											...
											if (map == null) {
												...
												if (std.isInfoEnabled()) {
													std.info("authenticateKeystone(): Putting (" + type + "," + map
															+ ") into services.");
												}
												services.put(type, map);
												bestVersion.put(type, verMap);
											}
											if (regionId == null & version.equals("1.0")
													&& !provider.getCloudProvider().equals(OpenStackProvider.RACKSPACE)) {
												std.warn("authenticateKeystone(): No region defined, making one up based on the URL: "
														+ url);
												regionId = toRegion(url);
												std.warn("authenticateKeystone(): Fabricated region is: " + regionId);
											} else if (regionId == null
													&& (type.equals("compute") || type.equals("object-store"))) {
											...
											if (std.isInfoEnabled()) {
												std.info("authenticateKeystone(): Comparing " + version + " against "
														+ verMap.get(type));
											}
											if (verMap.get(type) == null || compareVersions(version, verMap.get(type)) >= 0) {
												if (std.isInfoEnabled()) {
													std.info("authenticateKeystone(): Putting (" + regionId + "," + url
															+ ") into the " + type + " map.");
												}
												verMap.put(type, version);
												map.put(regionId, url);
											} else {
												std.warn("authenticateKeystone(): Skipping lower version url " + url
														+ " for " + type + " map.");
											}
						...
						// TODO: remove this when HP has the DBaaS in the
						// service catalog
						if (provider.getCloudProvider().equals(OpenStackProvider.HP)
								&& provider.getContext().getAccountNumber().equals("66565797737008")) {
							HashMap<String, String> endpoints = new HashMap<String, String>();

							endpoints.put("region-a.geo-1",
									"https://region-a.geo-1.dbaas-mysql.hpcloudsvc.com:8779/v1.0/66565797737008");
	...
	private @Nullable AuthenticationContext authenticateStandard(@Nonnull String endpointUrls) throws CloudException,
			InternalException {
		Logger std = NovaOpenStack.getLogger(NovaOpenStack.class, "std");
		Logger wire = NovaOpenStack.getLogger(NovaOpenStack.class, "wire");
	...
	private @Nullable AuthenticationContext authenticateSwift(@Nonnull String endpoint) throws CloudException,
			InternalException {
		Logger std = NovaOpenStack.getLogger(NovaOpenStack.class, "std");
		Logger wire = NovaOpenStack.getLogger(NovaOpenStack.class, "wire");
	...
	@Override
	public void deleteResource(@Nonnull final String service, @Nonnull final String resource,
			@Nonnull final String resourceId, @Nullable final String suffix) throws CloudException, InternalException {
	...
	@Override
	protected void delete(@Nonnull final String authToken, @Nonnull final String endpoint, @Nonnull final String resource)
	...
	@Override
	public @Nullable JSONObject getResource(@Nonnull final String service, @Nonnull final String resource,
			@Nullable final String resourceId, final boolean suffix) throws CloudException, InternalException {
		AuthenticationContext context = provider.getAuthenticationContext();
		String endpoint = context.getServiceUrl(service);
	...
	@Override
	protected @Nullable String getString(@Nonnull String authToken, @Nonnull String endpoint, @Nonnull String resource)
			throws CloudException, InternalException {
		Logger std = NovaOpenStack.getLogger(NovaOpenStack.class, "std");
		Logger wire = NovaOpenStack.getLogger(NovaOpenStack.class, "wire");
		...
		try {
			...
			if (code == HttpStatus.SC_BAD_REQUEST) {
				...
				if (provider.getMajorVersion() == 1 && provider.getMinorVersion() == 0 && items.message != null
						&& (items.message.contains("not found") || items.message.contains("unknown"))) {
					return null;
				}
	...
	@Override
	protected @Nullable InputStream getStream(@Nonnull String authToken, @Nonnull String endpoint, @Nonnull String resource)
			throws CloudException, InternalException {
		Logger std = NovaOpenStack.getLogger(NovaOpenStack.class, "std");
		Logger wire = NovaOpenStack.getLogger(NovaOpenStack.class, "wire");
	...
	@Override
	public @Nullable Map<String, String> headResource(@Nonnull final String service, @Nullable final String resource,
			@Nullable final String resourceId) throws CloudException, InternalException {
		AuthenticationContext context = provider.getAuthenticationContext();
		String endpoint = context.getServiceUrl(service);
	...
	@Override
	public void postResourceHeaders(final String service, final String resource, final String resourceId,
			final Map<String, String> headers) throws CloudException, InternalException {
		AuthenticationContext context = provider.getAuthenticationContext();
		String endpoint = context.getServiceUrl(service);
	...
	protected @Nullable String postHeaders(@Nonnull String authToken, @Nonnull String endpoint, @Nonnull String resource,
			@Nonnull Map<String, String> customHeaders) throws CloudException, InternalException {
		Logger std = NovaOpenStack.getLogger(NovaOpenStack.class, "std");
		Logger wire = NovaOpenStack.getLogger(NovaOpenStack.class, "wire");
	...
	@Override
	public @Nullable JSONObject postString(@Nonnull final String service, @Nonnull final String resource,
			@Nullable final String resourceId, @Nonnull final String extra, @Nonnull final JSONObject body)
			throws CloudException, InternalException {
		AuthenticationContext context = provider.getAuthenticationContext();
		String endpoint = context.getServiceUrl(service);
	...
	@Override
	public @Nullable JSONObject postString(@Nonnull final String service, @Nonnull final String resource,
			@Nullable final String resourceId, @Nonnull final JSONObject body, final boolean suffix) throws CloudException,
			InternalException {
		AuthenticationContext context = provider.getAuthenticationContext();
	...
	@Override
	protected @Nullable String postString(@Nonnull String authToken, @Nonnull String endpoint, @Nonnull String resource,
			@Nonnull String payload) throws CloudException, InternalException {
		Logger std = NovaOpenStack.getLogger(NovaOpenStack.class, "std");
		Logger wire = NovaOpenStack.getLogger(NovaOpenStack.class, "wire");
	...
	@Override
	@SuppressWarnings("unused")
	protected @Nullable String postStream(@Nonnull String authToken, @Nonnull String endpoint, @Nonnull String resource,
			@Nonnull String md5Hash, @Nonnull InputStream stream) throws CloudException, InternalException {
		Logger std = NovaOpenStack.getLogger(NovaOpenStack.class, "std");
		Logger wire = NovaOpenStack.getLogger(NovaOpenStack.class, "wire");
	...
	@Override
	public void putResourceHeaders(@Nonnull final String service, @Nullable final String resource,
			@Nullable final String resourceId, @Nonnull final Map<String, String> headers) throws CloudException,
			InternalException {
		AuthenticationContext context = provider.getAuthenticationContext();
		String endpoint = context.getServiceUrl(service);
	...
	@Override
	@SuppressWarnings("unused")
	protected @Nonnull String putHeaders(@Nonnull String authToken, @Nonnull String endpoint, @Nonnull String resource,
			@Nonnull Map<String, String> customHeaders) throws CloudException, InternalException {
		Logger std = NovaOpenStack.getLogger(NovaOpenStack.class, "std");
		Logger wire = NovaOpenStack.getLogger(NovaOpenStack.class, "wire");
	...
	@Override
	public @Nullable JSONObject putString(@Nonnull final String service, @Nonnull final String resource,
			@Nullable final String resourceId, @Nonnull final JSONObject body, final String suffix) throws CloudException,
			InternalException {
		AuthenticationContext context = provider.getAuthenticationContext();
		String resourceUri = resource;
		if (resourceId != null) {
	...
	@Override
	protected @Nullable String putString(@Nonnull String authToken, @Nonnull String endpoint, @Nonnull String resource,
	...
	@Override
	protected @Nullable String putStream(@Nonnull String authToken, @Nonnull String endpoint, @Nonnull String resource,
			@Nullable String md5Hash, @Nonnull InputStream stream) throws CloudException, InternalException {
		Logger std = NovaOpenStack.getLogger(NovaOpenStack.class, "std");
		Logger wire = NovaOpenStack.getLogger(NovaOpenStack.class, "wire");
	...
	private @Nonnegative int compareVersions(@Nullable String ver1, @Nullable String ver2) throws InternalException {
		...
		// Assumes only x.x granularity. Anything more will require rewrite of
		// this component.
		try {
			...
			if (logger.isTraceEnabled()) {
				logger.trace("exit - " + AbstractMethod.class.getName() + ".compareVersions(" + ver1 + "," + ver2 + ") = "
						+ result);
			}
		}
