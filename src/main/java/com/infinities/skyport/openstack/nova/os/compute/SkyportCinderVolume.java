...
import org.dasein.cloud.openstack.nova.os.compute.CinderVolumeCapabilities;
 ...
 *
 */
public class SkyportCinderVolume extends AbstractVolumeSupport<NovaOpenStack> {
	...
	public SkyportCinderVolume(@Nonnull NovaOpenStack provider) {
		super(provider);
	}
	...
	private @Nonnull String getResource() {
		// 20130930 dmayne: seems like hp may have upgraded and uses the same
		// resource
		return "/volumes";
		// return (((NovaOpenStack)getProvider()).isHP() ? "/os-volumes" :
		// "/volumes");
	}
	...
	@Override
	public void attach(@Nonnull String volumeId, @Nonnull String toServer, @Nonnull String device) throws InternalException,
			CloudException {
		...
		try {
			...
			if (method.postString(NovaServer.SERVICE, "/servers", toServer, getAttachmentsResource(),
					new JSONObject(wrapper)) == null) {
	...
	public @Nonnull String createVolume(@Nonnull VolumeCreateOptions options) throws InternalException, CloudException {
		if (options.getVlanId() != null) {
			throw new OperationNotSupportedException("Creating NFS volumes is not supported in "
					+ getProvider().getCloudName());
		}
	...
	public void detach(@Nonnull String volumeId, boolean force) throws InternalException, CloudException {
		...
		try {
			...
			method.deleteResource(NovaServer.SERVICE, "/servers", volume.getProviderVirtualMachineId(),
					getAttachmentsResource() + "/" + volumeId);
		} finally {
			APITrace.end();
		}
	}
	...
	@Override
	public @Nonnull Iterable<String> listPossibleDeviceIds(@Nonnull Platform platform) throws InternalException,
			CloudException {
	...
	public @Nonnull Iterable<VolumeProduct> listVolumeProducts() throws InternalException, CloudException {
		...
		try {
			Cache<VolumeProduct> cache =
					Cache.getInstance(getProvider(), "volumeProducts", VolumeProduct.class, CacheLevel.REGION_ACCOUNT);
			...
			if (json != null && json.has("volume_types")) {
				try {
					...
					throw new CloudException(CloudErrorType.COMMUNICATION, 200, "invalidJson",
							"Missing JSON element for volumes in " + json.toString());
				}
			}
	...
	private @Nullable Volume toVolume(@Nullable JSONObject json, @Nonnull Iterable<VolumeProduct> types)
		...
		try {
			...
			if (json.has("availability_zone") && json.getString("availability_zone") != null
					&& !json.getString("availability_zone").isEmpty()) {
			...
			long created = (json.has("createdAt") ? NovaOpenStack.parseTimestamp(json.getString("createdAt")) : -1L);

			if (created < 1L) {
				created = (json.has("created_at") ? NovaOpenStack.parseTimestamp(json.getString("created_at")) : -1L);
			}
			...
			JSONObject md = (json.has("metadata") && !json.isNull("metadata")) ? json.getJSONObject("metadata") : null;
